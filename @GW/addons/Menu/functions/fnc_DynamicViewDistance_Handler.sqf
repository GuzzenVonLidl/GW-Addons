...
if ((count GVAR(DynamicViewDistance_getAvgFPS)) isEqualTo 10) then {
	...
	_getAvg = (GVAR(DynamicViewDistance_getAvgFPS) call EFUNC(main,findAverage));
	GVAR(DynamicViewDistance_getAvgFPS) = [];
	TRACE_1("FPS",_getAvg);
	_diff = 50;
	_changeView = 0;	// 0: keep 1: Increase -1: Decrease
	if ((_getAvg >= (GVAR(DynamicViewDistance_AvgTargetDistance) + 5)) && (viewDistance < 3000)) then {
		_changeView = 1;
		_changeTo = (viewDistance + _diff);
	};

	if ((_getAvg <= (GVAR(DynamicViewDistance_AvgTargetDistance) - 5)) && (viewDistance > 500)) then {
		_changeView = -1;
		_changeTo = (viewDistance - _diff);
	};
