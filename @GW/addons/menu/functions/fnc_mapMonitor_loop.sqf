...
while {GVAR(mapMonitor_Enabled)} do {
	private ["_doneObjs", "_sigTerm"];
	_doneObjs = [];
	waitUntil {
		_sigTerm = GVAR(mapMonitor_sigTerm);
		visibleMap || {shownGPS} || {GVAR(mapMonitor_constantUpdate)} || {_sigTerm};
	};
	...
	{ // forEach
		...
		if ( // Object has changed state or script is ending
			...
			{!([(alive _obj), _alive] call FUNC(mapMonitor_compareBool))} || // Died or was revived
			{!([(isPlayer _obj), _isPlayer] call FUNC(mapMonitor_compareBool))} // Changed playable status
		) then { // Delete marker
	...
	if (visibleMap || GVAR(mapMonitor_constantUpdate)) then { // Repaint map markers

	/* Paint unit map markers */
	{ // forEach
		if (alive _x) then {
			[_x] call FUNC(mapMonitor_addFiredEH);
			private ["_playerStat"];
			_playerStat = _x getVariable QGVAR(mapMonitor_playerStat);
			if ((isNil "_playerStat") || {!([(isPlayer _x), (_playerStat select 1)] call FUNC(mapMonitor_compareBool))}) then {
				_x setVariable [QGVAR(mapMonitor_playerStat), [(name _x), (isPlayer _x)], true];
			};
			if (!(_x in _doneObjs) && {(vehicle _x) == _x}) then {
				private ["_color", "_type", "_marker"];
				_color = [side _x] call FUNC(mapMonitor_getSideColor);
				_type = if (isPlayer _x) then {"mil_arrow2"} else {"mil_arrow"};
				_marker = [_markers, _x, _type, _color, [0.4, 0.4], MARKER_ALPHA] call FUNC(mapMonitor_paintObjMarker);
	...
	{ // forEach
		if (!(_x in _doneObjs)) then {
			...
			_marker = [_markers, _x, "mil_arrow", "ColorBlack", [0.4, 0.4], MARKER_ALPHA] call FUNC(mapMonitor_paintObjMarker);
			_playerStat = _x getVariable QGVAR(mapMonitor_playerStat);
	...
	{ // forEach
		if (_x isKindOf "AllVehicles") then { // Protect against fake vehicles
			if (alive _x) then {
				if !(_x getVariable [QGVAR(mapMonitor_firedEH), false]) then {
					_x setVariable [QGVAR(mapMonitor_firedEH),true];
					_x addEventHandler ["Fired", {_this call FUNC(mapMonitor_firedEH);}];
				};
			//	[_x] call FUNC(mapMonitor_addFiredEH);
			};
			if (!(_x in _doneObjs)) then {
				...
				_marker = [_markers, _x, _type, _color, [0.8, 0.8], MARKER_ALPHA] call FUNC(mapMonitor_paintObjMarker);
				if (_detailLevel >= 1) then {
